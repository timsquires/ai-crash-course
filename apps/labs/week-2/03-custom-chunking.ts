...
function buildSectionRegex(): RegExp {
  const escaped = RESTAURANTS.flatMap((r) => r.aliases).map((a) => a.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
 ...
 */
function splitByRestaurantSections(text: string): Array<{ heading: string; start: number; end: number; body: string }> {
  ...
  const sections: Array<{ heading: string; start: number; end: number; body: string }> = [];
...
function resolveRestaurant(heading: string): { restaurant: string; aliasesMatched: string[] } | null {
  const headingLower = heading.toLowerCase();
  for (const r of RESTAURANTS) {
    const matched = r.aliases.filter((a) => headingLower === a.toLowerCase());
    if (matched.length > 0) return { restaurant: r.name, aliasesMatched: matched };
  }
  return null;
}
...
export default async function main() {
  ...
  const perRestaurantCounts: Record<string, number> = Object.fromEntries(RESTAURANTS.map(r => [r.name, 0]));
  ...
  for (const e of entries) {
    ...
    for (const sec of sections) {
      ...
      const headingRe = new RegExp('^\\s*' + escapeRegex(sec.heading) + '\\s*', 'i');
      ...
      perRestaurantCounts[resolved.restaurant] = (perRestaurantCounts[resolved.restaurant] || 0) + 1;
    }
  }
