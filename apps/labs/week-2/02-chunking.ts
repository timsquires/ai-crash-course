...
type Chunk = {
  ...
  mimeType: string;
  page?: number;
  version: { lab: string; encoder: string; chunkTokens: number; overlapTokens: number };
};
...
async function readIndex(indexPath: string): Promise<IndexEntry[]> {
  ...
  const entries = JSON.parse(buf) as Array<IndexEntry & Record<string, unknown>>;
  // Keep only the fields we care about
  return entries.map((e) => ({ id: String(e.id), sourcePath: String(e.sourcePath), mimeType: String(e.mimeType), page: (e as any).page }));
}

async function readDocText(baseDir: string, id: string): Promise<string | null> {
...
export default async function main() {
  ...
  if (!fs.existsSync(indexPath)) {
    console.error('Missing Lab 1 output. Run week-2/01-document-loading first.');
    return;
  }
  ...
  let totalTokens = 0;
  let totalDocs = 0;
  const perDocStats: Array<{ id: string; tokens: number; chunks: number; avgChunkTokens: number }> = [];
  ...
  try {
    for (const e of entries) {
      ...
      const overlapTokens = Math.min(Math.max(0, OVERLAP_TOKENS), Math.max(0, chunkTokens - 1));
      ...
      for (let start = 0; start < tokenCount; start += step) {
        ...
        const content = typeof decoded === 'string' ? decoded : new TextDecoder('utf-8').decode(decoded as Uint8Array);
        ...
        const outChunk: Chunk = {
          ...
          mimeType: e.mimeType,
          page: e.page,
          version: { lab: '02-chunking', encoder: String(MODEL_FOR_TOKENIZATION), chunkTokens, overlapTokens },
        };
      ...
      perDocStats.push({ id: e.id, tokens: tokenCount, chunks: docChunkCount, avgChunkTokens: docChunkCount ? Math.round(tokenCount / docChunkCount) : 0 });
  ...
  const avgTokensPerChunk = totalChunks ? Math.round(totalTokens / totalChunks) : 0;
  const manifest = {
    lab: '02-chunking',
    encoder: MODEL_FOR_TOKENIZATION,
    config: { CHUNK_TOKENS, OVERLAP_TOKENS },
    totals: { totalDocs, totalChunks, totalTokens, avgTokensPerDoc, avgTokensPerChunk },
    perDoc: perDocStats,
  };
