...
function getRestaurantStats(chain: string): RestaurantStats | null {
  ...
  if (key.includes('in') && key.includes('out')) return RESTAURANT_DB['in-n-out'] ?? null;
...
const getRestaurantStatsTool = tool(
  ...
  async (input: unknown) => {    
    const chain = typeof input === 'object' && input && 'chain' in (input as any) ? (input as any).chain : '';
...
function resolveProvider(): Provider {
  ...
  if (cand === 'openai' || cand === 'claude' || cand === 'gemini' || cand === 'grok') return cand;
  return 'openai';
}
...
function checkProviderEnv(provider: Provider): { ok: boolean; message?: string } {
  switch (provider) {
    case 'openai':
      return { ok: !!process.env.OPENAI_API_KEY, message: 'Set OPENAI_API_KEY in apps/labs/.env' };
    case 'claude':
      return { ok: !!process.env.ANTHROPIC_API_KEY, message: 'Set ANTHROPIC_API_KEY in apps/labs/.env' };
    case 'gemini':
      return { ok: !!process.env.GOOGLE_API_KEY, message: 'Set GOOGLE_API_KEY in apps/labs/.env' };
    case 'grok':
      return { ok: !!process.env.XAI_API_KEY, message: 'Set XAI_API_KEY in apps/labs/.env' };
  }
}
...
export default async function main() {
  ...
  if (!apiKeyPresent) {
    console.error('Set provider API keys in apps/labs/.env (OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY, or XAI_API_KEY).');
    return;
  }
  ...
  const userPrompt = 'Compare the healthiness and value of Chipotle and Chick-fil-A, and pick a winner.';
  ...
  for (const tc of toolCalls) {    
    try {
      if (tc.name === 'get_restaurant_stats') {        
        ...
        messages.push(new ToolMessage({ content: JSON.stringify(result.content ?? null), tool_call_id: tc.id ?? '' }));
      ...
      messages.push(new ToolMessage({content: JSON.stringify({ error: 'Failed to run tool', rawArgs: tc.args }), tool_call_id: tc.id ?? ''}));
    }    
  }
