...
export class ChatWidget extends LitElement {
  ...
  @property({ type: Object }) threadParameters: Record<string, unknown> | null =
    null;
  ...
  static styles = css`
    ...
    :host([theme='dark']) input::placeholder {
      color: #aaa;
    }
      ...
      <div class="container">
        <div class="header">
          ...
          ${this.isOpen
            ...
            : html`<button title="Open" @click=${() => (this.isOpen = true)}>
                +
              </button>`}
        </div>
        ${this.isOpen
          ? html`
              <div class="body">
                ${this.messages.length === 0 && !this.runLLMCallOnInit
              ...
              <div class="footer">
                ...
                <button
                  @click=${this.onSend}
                  ?disabled=${!this.input.trim() || this.isLoading}
                >
                  Send
                </button>
              </div>
  ...
  private async onSend() {
    ...
    try {
      ...
      const data: AssistantReply = await this.api!.sendMessage(
        this.threadId!,
        text,
      );
      ...
      const errorMessage =
        e instanceof Error ? e.message : 'Sorry, something went wrong.';
      this.add('assistant', `Error: ${errorMessage}`);
    } finally {
  ...
  private async ensureThread() {
    this.threadId = await this.api!.createThread(
      this.agent,
      this.threadParameters || undefined,
    );
  }
  ...
  private async fetchGreeting() {
    try {
      ...
      const data: AssistantReply = await this.api!.sendMessage(
        this.threadId!,
        'Please greet the user to begin the conversation. Keep it brief and friendly.',
      );
