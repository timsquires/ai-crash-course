...
export function initializeChatWidget(config: ChatWidgetConfig): ChatWidget {
  ...
  if (existing) {
    ...
    if (typeof config.ragEnabled === "boolean")
      existing.ragEnabled = config.ragEnabled;
    ...
    if (typeof config.runLLMCallOnInit === "boolean")
      existing.runLLMCallOnInit = config.runLLMCallOnInit;
    return existing;
  }
  ...
  if (typeof config.runLLMCallOnInit === "boolean")
    el.runLLMCallOnInit = config.runLLMCallOnInit;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const config: ChatWidgetConfig = {
    ...
    runLLMCallOnInit:
      (script.getAttribute("data-run-llm-init") || "false") === "true",
  };
