...
export class PgDocumentRepository implements DocumentRepository {
  ...
  async create(accountId: string, filename: string, mimeType: string): Promise<DocumentRecord> {
    const entity = this.repo.create({ id: randomUUID(), accountId, filename, mimeType });
    const saved = await this.repo.save(entity);
    return { id: saved.id, accountId: saved.accountId, filename: saved.filename, mimeType: saved.mimeType, createdAt: saved.createdAt };
  }
