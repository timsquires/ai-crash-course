...
export class PgChunkRepository implements ChunkRepository {
  constructor(@InjectRepository(ChunkEntity) private readonly repo: Repository<ChunkEntity>) {}
  ...
  async bulkCreate(chunks: Omit<ChunkRecord, 'id' | 'createdAt'>[]): Promise<void> {
    ...
    for (let i = 0; i < baseRows.length; i++) {
      ...
      await this.repo.query('UPDATE chunks SET embedding_vec = $1::vector WHERE id = $2', [vec, id]);
    }
  }
